        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-5">
          {industries.map((industry, index) => (
            <div
              ...
              className={`group relative p-6 rounded-2xl transition-all duration-500 cursor-pointer overflow-hidden ${hoveredIndex === index
                  ? "bg-[#1F4FD8] shadow-2xl shadow-[#1F4FD8]/30"
                  : "bg-white hover:shadow-xl"
                }`}
            >
              {/* Background decoration */}
              <div className={`absolute -right-8 -bottom-8 w-32 h-32 rounded-full transition-all duration-500 ${hoveredIndex === index ? "bg-white/10" : "bg-[#1F4FD8]/5"
                }`} />

              {/* Content */}
              <div className="relative z-10">
                {/* Icon */}
                <div className={`w-14 h-14 rounded-xl flex items-center justify-center mb-4 transition-all duration-300 ${hoveredIndex === index
                ...
                <div className="flex items-center justify-between mb-2">
                  <h3 className={`text-lg font-bold transition-colors duration-300 ${hoveredIndex === index ? "text-white" : "primary-black"
                    }`}>
                    {industry.name}
                  </h3>
                  <span className={`text-xs font-semibold px-2 py-1 rounded-full transition-all duration-300 ${hoveredIndex === index
                      ? "bg-white/20 text-white"
                      : "bg-[#1F4FD8]/10 text-[#1F4FD8]"
                    }`}>
                    {industry.apps}
                ...
                {/* Description */}
                <p className={`text-sm leading-relaxed transition-colors duration-300 ${hoveredIndex === index ? "text-white/80" : "primary-black/60"
                  }`}>
                  {industry.description}
                </p>

                {/* Arrow */}
                <div className={`mt-4 flex items-center gap-2 text-sm font-semibold transition-all duration-300 ${hoveredIndex === index
