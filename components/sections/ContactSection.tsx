...
type FieldKey = "fullName" | "email" | "phone" | "companyName" | "requirement";

/**
 * Contact form UX and security model:
 * - `contactFormSchema` validates visible fields client-side first; POST /api/contact validates again.
 * - `CONTACT_HONEYPOT_FIELD` is visually hidden—humans leave it blank; naive bots fill it and get a silent
 *   fake success server-side (`lib/contact/honeypot.ts`).
 * - Rate limits / IP blocks run only on the server (`lib/contact/rate-limit.ts`).
 */
export default function ContactSection() {
  const [status, setStatus] = useState<"idle" | "loading" | "success" | "error">("idle");
  const [errorMessage, setErrorMessage] = useState("");
  const [fieldErrors, setFieldErrors] = useState<Partial<Record<FieldKey, string>>>({});
  const [requirementPrefill, setRequirementPrefill] = useState("");

  useEffect(() => {
    const interest = new URLSearchParams(window.location.search).get("interest");
    if (interest) {
      setRequirementPrefill(interest);
    }
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const honeypot = String(formData.get(CONTACT_HONEYPOT_FIELD) ?? "");
    ...
    const parsed = contactFormSchema.safeParse(bodyForValidation);
    if (!parsed.success) {
  ...
  return (
    <section
      id="contact"
      className="w-full relative overflow-hidden secondary-background py-12 sm:py-16 md:py-20 lg:py-24 scroll-mt-24 md:scroll-mt-28"
    >
      {/* Background Image */}
      <Image
          ...
          <div className="space-y-4">
            <div>
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  <label htmlFor="fullName" className="block mb-2 text-xs sm:text-sm font-medium primary-black">Full Name*</label>
                  <div className="relative">
                    ...
                    <input type="text" id="fullName" name="fullName" autoComplete="name" maxLength={100} className="block w-full ps-8 sm:ps-9 pe-3 py-2.5 sm:py-3 bg-neutral-secondary-medium border neutral-grey-border text-neutral-900 text-xs sm:text-sm rounded-base focus:ring-brand focus:border-brand shadow-xs placeholder:text-neutral-500 rounded-lg" placeholder="Full Name" aria-invalid={fieldErrors.fullName ? true : undefined} aria-describedby={fieldErrors.fullName ? "fullName-error" : undefined} />
                  </div>
                  {fieldErrors.fullName ? (
                    <p id="fullName-error" className="mt-1 text-xs text-red-600" role="alert">{fieldErrors.fullName}</p>
                  ) : null}
                </div>

                <div>
                  <label htmlFor="email" className="block mb-2 sm:mb-2.5 text-xs sm:text-sm font-medium primary-black">Email*</label>
                  <div className="relative">
                    <div className="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                    ...
                    <input type="email" id="email" name="email" autoComplete="email" maxLength={254} className="block w-full ps-8 sm:ps-9 pe-3 py-2.5 sm:py-3 bg-neutral-secondary-medium border neutral-grey-border text-neutral-900 text-xs sm:text-sm rounded-base focus:ring-brand focus:border-brand shadow-xs placeholder:text-neutral-500 rounded-lg" placeholder="Email" aria-invalid={fieldErrors.email ? true : undefined} aria-describedby={fieldErrors.email ? "email-error" : undefined} />
                  </div>
                  {fieldErrors.email ? (
            ...
            <div className="grid gap-3 sm:gap-4 md:grid-cols-2">
              <div>
                <label htmlFor="phone" className="block mb-2 sm:mb-2.5 text-xs sm:text-sm font-medium primary-black">Phone Number*</label>
                <div className="relative">
                  <div className="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                  ...
                  <input type="tel" id="phone" name="phone" autoComplete="tel" maxLength={40} inputMode="tel" className="block w-full ps-8 sm:ps-9 pe-3 py-2.5 sm:py-3 bg-neutral-secondary-medium border neutral-grey-border text-neutral-900 text-xs sm:text-sm rounded-base focus:ring-brand focus:border-brand shadow-xs placeholder:text-neutral-500 rounded-lg" placeholder="Phone Number" aria-invalid={fieldErrors.phone ? true : undefined} aria-describedby={fieldErrors.phone ? "phone-error" : undefined} />
                </div>
                {fieldErrors.phone ? (
              ...
              <div>
                <label htmlFor="companyName" className="block mb-2 sm:mb-2.5 text-xs sm:text-sm font-medium primary-black">Company Name*</label>
                <div className="relative">
                  <div className="absolute inset-y-0 start-0 flex items-center ps-3 pointer-events-none">
                  ...
                  <input type="text" id="companyName" name="companyName" autoComplete="organization" maxLength={200} className="block w-full ps-8 sm:ps-9 pe-3 py-2.5 sm:py-3 bg-neutral-secondary-medium border neutral-grey-border text-neutral-900 text-xs sm:text-sm rounded-base focus:ring-brand focus:border-brand shadow-xs placeholder:text-neutral-500 rounded-lg" placeholder="PRNIT India LLP" aria-invalid={fieldErrors.companyName ? true : undefined} aria-describedby={fieldErrors.companyName ? "companyName-error" : undefined} />
                </div>
                {fieldErrors.companyName ? (
                  <p id="companyName-error" className="mt-1 text-xs text-red-600" role="alert">{fieldErrors.companyName}</p>
                ) : null}
              </div>
            </div>

            <div>
              <label htmlFor="requirement" className="block mb-2 sm:mb-2.5 text-xs sm:text-sm font-medium primary-black">Your Requirement*</label>
              <div className="relative">
                ...
                <textarea id="requirement" name="requirement" minLength={5} maxLength={5000} defaultValue={requirementPrefill} className="block w-full ps-8 sm:ps-9 pe-3 py-2.5 sm:py-3 bg-neutral-secondary-medium border neutral-grey-border text-neutral-900 text-xs sm:text-sm rounded-base focus:ring-brand focus:border-brand shadow-xs placeholder:text-neutral-500 rounded-lg" rows={5} placeholder="Tell us about your project" aria-invalid={fieldErrors.requirement ? true : undefined} aria-describedby={fieldErrors.requirement ? "requirement-error" : undefined} />
              </div>
              {fieldErrors.requirement ? (
                <p id="requirement-error" className="mt-1 text-xs text-red-600" role="alert">{fieldErrors.requirement}</p>
              ) : null}
            </div>

            {/* Honeypot — leave empty; bots that fill this field get a harmless success response server-side (see lib/contact/honeypot.ts). */}
            <input
              type="text"
              ...
              autoComplete="off"
              defaultValue=""
              className="absolute start-[-9999px] top-auto h-px w-px overflow-hidden border-0 p-0 opacity-0"
              aria-hidden
            />
